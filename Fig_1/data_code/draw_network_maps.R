...
svg("Ecy_net.svg", width = 5, height = 5); draw.split.network(Nnet.ecy); dev.off()
svg("Eve_net.svg", width = 20, height = 20); draw.split.network(Nnet.eve); dev.off()
svg("Gfa_net.svg", width = 10, height = 10); draw.split.network(Nnet.gfa); dev.off()
...
ggplot(data = russia) +
  ...
  geom_point(data = coord.info, aes(x = lon, y = lat, 
                                    fill = bin_uri, shape = species_name), size = 2) +
  coord_sf(xlim = c(102.5, 110.5), ylim = c(51, 56), expand = FALSE) + 
  scale_shape_manual(values = c("Gmelinoides fasciatus" = 24,
                              "Eulimnogammarus verrucosus" = 22,
                              "Eulimnogammarus cyaneus" = 23)) + 
  scale_fill_manual(values = c("BOLD:ADC7414"="#F0E442", 
                               "BOLD:AEB1648" = "#ea568c", 
                               "BOLD:AAY3465" = "#D81B60", 
                               "BOLD:AEB1649" = "#4477AA", 
                    ...
                    guide = guide_legend(override.aes = list(shape=21,
                                                             alpha = 1)))+ 
  facet_wrap(facets = . ~ species_name) + 
  theme_minimal() + 
  theme(legend.position = 'none')

svg("map.svg", width = 10, height = 5); map; dev.off()
